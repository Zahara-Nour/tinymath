...
<img
	id="tuteur_index"
	alt="tuteur_index"
	src={`/images/navadra/personnages/${$player.profile.tutor.toLowerCase()}.png`}
/>
...
{#each $player.monsters as monster}
	...
	<a href={`/navadra/Fight?monsterId=${monster.profile.id}`}>
		...
		>
			<img
				...
				src={`/images/navadra/monstres/${monster.profile.name.toLowerCase()}_tete.png`}
			/>
		</div>
